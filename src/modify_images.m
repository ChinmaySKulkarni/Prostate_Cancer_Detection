function [modified_images,filenames] = modify_images(write_flag, name)
    [image_data,~,filenames, img_x, img_y, numcolors] = load_data(strcat('../pictures_data/',name),1);
    ...
    for i=1:total_images
        ...
        if write_flag == 1
            temp = strcat('../modified_', name);
            imwrite(squeeze(modified_images(i,:,:,:)), strcat(temp, filenames{i}));
        end
    end